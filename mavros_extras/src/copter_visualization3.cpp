...
static void publish_track_marker(const geometry_msgs::PoseStamped::ConstPtr &pose)
{
	...
	track_marker->header.frame_id = fixed_frame_id;
...
static void publish_wp_marker(const geometry_msgs::PoseStamped::ConstPtr &wp)
{
	...
	if ( !marker )	// only instantiate marker once
	{
		...
		marker->scale.x = marker_scale * 0.08;//1.0;
		marker->scale.y = marker_scale * 0.08;//0.1;
...
{
	ros::init(argc, argv, "copter_visualization3");
	ros::NodeHandle nh;
	...
	priv_nh.param<std::string>("child_frame_id", child_frame_id, "fcu3");
	...
	track_marker_pub = nh.advertise<visualization_msgs::Marker>("track_markers3", 10);
	vehicle_marker_pub = nh.advertise<visualization_msgs::MarkerArray>("vehicle_marker3", 10);
	wp_marker_pub = nh.advertise<visualization_msgs::Marker>("wp_markers3", 10);

	auto pos_sub = nh.subscribe("/mavros3/local_position/pose", 10, local_position_sub_cb);
	auto wp_sub = nh.subscribe("/mavros3/setpoint_raw/target_local_rviz", 10, setpoint_local_pos_sub_cb);
